...
struct SFGP
    ...
    function SFGP(gp::SVGP,n_points=10,limit=3)
        ...
        vals = σ.(collect(range(-limit,limit,length=n_points)))
    
    return new(gp,lower,upper,vals)
...
Flux.params(m::SFGP) = Flux.params(m.gp)
...
function lpdf(mm::SFGP,x,y)
    ...
    return sum(probs .* logpdf.(Bernoulli.(sigvals),y))
