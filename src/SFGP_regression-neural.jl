...
struct SFGP
    ...
    function SFGP(gp::SVGP,net::Chain,n_points=10,limit=3)
    
        lower = vcat(-1e10,collect(range(-limit,limit,length=n_points-1)))[:,:]
        upper = vcat(collect(range(-limit,limit,length=n_points-1)),1e10)[:,:]

        net.layers[end].b[:] .= (collect(range(-limit,limit,length=n_points)))
        net.layers[end].W .*= 0.01
    
    return new(gp,lower,upper,net,ones(1,1))
...
Flux.params(m::SFGP) = Flux.params(m.gp,m.net,m.s)
...
function lpdf(mm::SFGP,x,y)
    ...
    return sum(probs .* lpdf_norm.(sigvals, abs(mm.s[1,1]),y))
